...
def triggerProductBuild(wso2_product, wso2_product_version) {
    try {
        ...
        } else if (wso2_product == "wso2-obam" || wso2_product == "wso2-obbi-worker" || wso2_product == "wso2-obbi-dashboard" || wso2_product == "wso2-obkm" || wso2_product == "wso2-obiam") {
...
def call() {
    node {
        stage('Get WUM released products') {
            ...
            { set +x; } 2>/dev/null
                rm -rf *
                touch successResult.html failResult.html wum_list.txt scan_list.txt
                echo "$RELEASED_PRODUCTS"  >> wum_list.txt
                echo "WUM Released Products\n-------------------------"
                cat wum_list.txt
            """
            ...
            for (i in arr) {
                ...
                if (itemCount == 3) {
                    wso2_product = i.tokenize("-")[0] + '-' + i.tokenize("-")[1]
                    wso2_version = i.tokenize("-")[2]
                } else {
                    wso2_product = i.tokenize("-")[0]
                    wso2_version = i.tokenize("-")[1]
                }
            ...
            script {
                try {
                    if (fileExists("successResult.html")) {
                        ...
                        String wumList = readFile "wum_list.txt"
                        ...
                        send("Docker Image Build Results! #(${env.BUILD_NUMBER}) - WSO2 IE Team", """
                            ...
                            <th>WUM Released Products</th></tr><tr>
                            <td>${wumList}</td></tr></table><br/>
